...
pub(crate) fn parse_file_param(input: TokenStream) -> Result<(String, std::fs::File), (Span, &'static str)> {
    ...
    let Some(path) = path.strip_prefix('"').and_then(|path| path.strip_suffix('"')) else {
        return Err((path_span, "Expected a string literal"));
    };
    if let Some(p) = input.next() {
        return Err((p.span(), "Unexpected token after file path"));
    };
    std::fs::File::open(path).map_err(|_| (path_span, "Failed to open file")).map(|x| (path.to_owned(), x))
}
