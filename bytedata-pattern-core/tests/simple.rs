...
const VERB_JOIN: Test = Test::join(&[
    VERB_HELLO,
    Test::char_class(CharacterClass::Whitespace),
    VERB_REPEAT,
]);
const VERB_JOIN_EXACT: Test = Test::join(&[
    Test::context_class(ContextClass::Start),
    VERB_HELLO,
    Test::char_class(CharacterClass::Whitespace),
    VERB_REPEAT,
    Test::context_class(ContextClass::End),
]);
...
fn simple_test_join() {
    ...
    VERB_JOIN.assert(
        "hello world ".as_bytes(),
        "expected to match on trailing whitespace",
    );
    VERB_JOIN.assert(
        " hello world".as_bytes(),
        "expected to match on leading whitespace",
    );
    VERB_JOIN.assert(
        " hello world ".as_bytes(),
        "expected to match on wrapped whitespace",
    );
    VERB_JOIN.assert(
        "hello worldssss".as_bytes(),
        "expected to match on trailing characters",
    );
    VERB_JOIN.assert_fail(
        "hello_world".as_bytes(),
        "expected to fail on missing whitespace",
    );
...
fn simple_test_join_exact() {
    ...
    VERB_JOIN_EXACT.assert_fail(
        "hello world ".as_bytes(),
        "expected to fail on trailing whitespace",
    );
    VERB_JOIN_EXACT.assert_fail(
        " hello world".as_bytes(),
        "expected to fail on leading whitespace",
    );
    VERB_JOIN_EXACT.assert_fail(
        " hello world ".as_bytes(),
        "expected to fail on wrapped whitespace",
    );
    VERB_JOIN_EXACT.assert_fail(
        "hello worldssss".as_bytes(),
        "expected to fail on trailing characters",
    );
    VERB_JOIN_EXACT.assert_fail(
        "hello_world".as_bytes(),
        "expected to fail on missing whitespace",
